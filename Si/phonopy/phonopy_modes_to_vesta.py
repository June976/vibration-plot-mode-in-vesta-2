...
def get_cell_lattice_abc(primitive_cell):
    primitive_cell_matrix = primitive_cell.get_cell()
    lat_a = np.linalg.norm(primitive_cell_matrix[0])
    lat_b = np.linalg.norm(primitive_cell_matrix[1])
    lat_c = np.linalg.norm(primitive_cell_matrix[2])
...
def trans_Eigvectors_To_Direct(mat_eigvecs, primitive_cell):
    primitive_cell_matrix = primitive_cell.get_cell()
    primitive_cell_matrix_to_direct = np.linalg.inv(primitive_cell_matrix)
    mat_eigvecs_direct = mat_eigvecs.copy()
    for i in range(mat_eigvecs.shape[0]):
        for j in range(mat_eigvecs.shape[1]):
            mat_eigvecs_direct[i, j] = np.dot(mat_eigvecs[i, j], primitive_cell_matrix_to_direct)
    return mat_eigvecs_direct


def write_Vesta_Mode(eigvecs, vesta_front, vesta_end, nat, scaling_factor, dir_out_file, lat_a, lat_b, lat_c):
    for index in range(nat*3):
        with open("{}/mode_{}.vesta".format(dir_out_file, index+1), 'w') as modef:
            ...
            for i in range(1,1+nat):
                towrite += "%4d%9.5f%9.5f%9.5f\n"%(i,eigvec[i-1][0]*lat_a*sf,eigvec[i-1][1]*lat_b*sf,eigvec[i-1][2]*lat_c*sf)
                towrite += "%5d  0   0    0    0\n 0 0 0 0 0\n"%i
            towrite += " 0 0 0 0 0\n"
            towrite += "VECTT\n"
            for i in range(1,1+nat):
...
if __name__ == '__main__':
    ...
    scaling_factor = 2                                          # The scale factor of the length of vector in VESTA file             
    supercell_matrix = [2, 2, 2]                                # The supercell matrix used for force sets calculation
    unitcell_filename = './POSCAR'                             # The unit cell used for force sets calculation
    ...
    primitive_cell = write_Primitive_Poscar(phonon_run, dir_out_file)
    nat, atom_cart = get_atom_cartesian_xyz(dir_out_file)
    lat_a, lat_b, lat_c = get_cell_lattice_abc(primitive_cell)
    ...
    eigvecs_direct = trans_Eigvectors_To_Direct(eigvecs, primitive_cell)
    write_Vesta_Mode(eigvecs_direct, vesta_front, vesta_end, nat, scaling_factor, dir_out_file, lat_a, lat_b, lat_c)
    write_Summary_Info_csv('phons_info.csv', eigvals, eigvecs_direct, atom_norms, atom_cart, norms, dir_out_file)
